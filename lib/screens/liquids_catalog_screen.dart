...
class _LiquidsCatalogScreenState extends State<LiquidsCatalogScreen> 
  ...
  Future<void> _showLiquidConfirmationDialog({
    ...
    if (result == true) {
      ...
      if (mounted) {
        Navigator.of(context).pop(true); // true означает, что данные изменились
      }
    }
  }
...
// ===== NEW ANIMATED STATUS CARD WITH WATER RING =====
    ...
    return Container(
      ...
      child: Column(
        children: [
          Row(
            children: [
              // Animated water ring - smaller size
              _WaterRingWithWave(
...
// ===== WATER RING WITH WAVE ANIMATION =====
class _WaterRingWithWave extends StatelessWidget {
  final int consumed;
  final int goal;
  final Color statusColor;
  final String units;
...
// ===== WATER RING WITH WAVE ANIMATION (from main screen) =====
