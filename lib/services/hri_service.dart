...
class HRIService extends ChangeNotifier {
  ...
  List<CaffeineIntake> get todayCaffeineIntakes =>
      List.unmodifiable(_todayCaffeineIntakes);
  ...
  static WorkoutLossResult calculateWorkoutLosses({
    ...
    final baseSweatRate =
        (item.properties['sweatRate'] as int? ?? 600); // ml/hour
    ...
    final adjustedSweatRate =
        baseSweatRate * (userWeight / 70) * (intensityValue / 3.0);
  ...
  Future<void> _loadData() async {
    ...
    for (final json in workoutsJson) {
      try {
        ...
        logger.e('Error loading workout: $e');
      }
    }
    ...
    for (final json in caffeineJson) {
      try {
        ...
        logger.e('Error loading caffeine: $e');
      }
    }
    ...
    if (componentsJson != null) {
      try {
        ...
        logger.e('Error loading components: $e');
      }
    }
  ...
  Future<void> _saveData() async {
    ...
    final workoutsJson = _todayWorkouts
        .map(
          (w) =>
    ...
    // Save caffeine
    final caffeineJson = _todayCaffeineIntakes
        .map(
          (c) =>
  ...
  Future<void> _saveWorkoutToFirestore(Workout workout) async {
    try {
      ...
      logger.i(
        'Workout saved to Firestore: ${workout.type} ${workout.durationMinutes}min',
      );
    } catch (e) {
  ...
  Future<void> _saveCaffeineToFirestore(CaffeineIntake intake) async {
    try {
      ...
      logger.i(
        'Caffeine saved to Firestore: ${intake.caffeineMg}mg from ${intake.source}',
      );
    } catch (e) {
  ...
  Future<void> addCaffeineIntake(
    double caffeineMg, {
    String source = 'coffee',
  }) async {
  ...
  Future<void> calculateHRI({
    ...
    double effectiveWaterIntake =
        waterIntake + foodWaterContent; // Add water from food
    ...
    final waterRatio = effectiveWaterGoal > 0
        ? (effectiveWaterIntake / effectiveWaterGoal)
        : 0.0;
    ...
    final sodiumRatio = effectiveSodiumGoal > 0
        ? (totalSodiumIntake / effectiveSodiumGoal)
        : 0.0;
    _components['sodium'] = _calculateElectrolyteComponent(
      sodiumRatio,
      maxPoints: 15,
    );

    final potassiumRatio = effectivePotassiumGoal > 0
        ? (potassiumIntake / effectivePotassiumGoal)
        : 0.0;
    _components['potassium'] = _calculateElectrolyteComponent(
      potassiumRatio,
      maxPoints: 10,
    );

    final magnesiumRatio = effectiveMagnesiumGoal > 0
        ? (magnesiumIntake / effectiveMagnesiumGoal)
        : 0.0;
    _components['magnesium'] = _calculateElectrolyteComponent(
      magnesiumRatio,
      maxPoints: 10,
    );
    ...
    if (_totalWaterLossMl > 0) {
      ...
      if (compensationRatio < 0.5) {
        ...
        _components['dehydration'] =
            10.0 * (1 - (compensationRatio - 0.5) / 0.3);
      } else if (compensationRatio < 1.0) {
        // Mild dehydration
        _components['dehydration'] =
            5.0 * (1 - (compensationRatio - 0.8) / 0.2);
      }
    }
    ...
    for (final workout in _todayWorkouts) {
      _components['workouts'] =
          (_components['workouts']! + workout.getHRIImpact());
    }
    ...
    if (totalSugarIntake > 0) {
      ...
      if (totalSugarIntake > sugarThreshold) {
        ...
        _components['sugar'] = (excess * sugarMultiplier).clamp(
          0,
          sugarMaxImpact,
        );
      }
    ...
    if (foodCount > 0) {
      ...
      if (foodSodiumIntake > 1000) {
        // Above 1g sodium from food
        _components['food'] =
            _components['food']! + min(5.0, (foodSodiumIntake - 1000) / 200);
      }

      // Food with low water content increases risk
      final avgWaterPerFood = foodCount > 0
          ? foodWaterContent / foodCount
          : 0.0;
      if (avgWaterPerFood < 50) {
        // Low water content foods (< 50ml per item average)
    ...
    double waterContribution =
        _components['water']! * 0.5; // 50% weight for water
    ...
    double otherContribution =
        otherComponents * 0.5; // 50% weight for everything else
    ...
    // Debug output
    if (kDebugMode) {
      logger.i('=== HRI Calculation Update (Weighted) ===');
      logger.i(
        'Water: ${_components['water']?.toStringAsFixed(1)} (ratio: ${waterRatio.toStringAsFixed(2)}) → ${waterContribution.toStringAsFixed(1)} (50% weight)',
      );
      logger.i('Sodium: ${_components['sodium']?.toStringAsFixed(1)}');
      logger.i('Potassium: ${_components['potassium']?.toStringAsFixed(1)}');
      logger.i('Magnesium: ${_components['magnesium']?.toStringAsFixed(1)}');
      logger.i('Heat: ${_components['heat']?.toStringAsFixed(1)}');
      logger.i(
        'Workouts: ${_components['workouts']?.toStringAsFixed(1)} (${_todayWorkouts.length} workouts)',
      );
      logger.i(
        'Dehydration: ${_components['dehydration']?.toStringAsFixed(1)} (loss: ${_totalWaterLossMl}ml)',
      );
      logger.i(
        'Caffeine: ${_components['caffeine']?.toStringAsFixed(1)} (${totalActiveCaffeine.toStringAsFixed(0)}mg active)',
      );
      logger.i(
        'Alcohol: ${_components['alcohol']?.toStringAsFixed(1)} (${_todayAlcoholSD.toStringAsFixed(1)} SD)',
      );
      logger.i(
        'Sugar: ${_components['sugar']?.toStringAsFixed(1)} (total: ${totalSugarIntake.toStringAsFixed(1)}g)',
      );
      logger.i(
        'Food: ${_components['food']?.toStringAsFixed(1)} (${foodCount} items, ${foodSodiumIntake.toStringAsFixed(0)}mg Na, ${foodWaterContent.toStringAsFixed(0)}ml H2O)',
      );
      logger.i('Time: ${_components['timeSinceIntake']?.toStringAsFixed(1)}');
      logger.i('Morning: ${_components['morning']?.toStringAsFixed(1)}');
      logger.i(
        'Other factors total: ${otherComponents.toStringAsFixed(1)} → ${otherContribution.toStringAsFixed(1)} (50% weight)',
      );
      logger.i(
        'TOTAL HRI: ${_currentHRI.toStringAsFixed(1)} (${waterContribution.toStringAsFixed(1)} + ${otherContribution.toStringAsFixed(1)})',
      );
      logger.i('==============================');
    }

    // Track HRI achievements
    _achievementTracker.trackHRI(hriValue: _currentHRI, status: hriStatus);
  ...
  Future<void> _saveEndOfDaySnapshot() async {
    try {
      ...
      logger.i('End-of-day HRI snapshot saved for ${_formatDate(yesterday)}');
    } catch (e) {
      logger.i('Error saving end-of-day HRI snapshot: $e');
    }
  }
  ...
  Future<void> saveCurrentDaySnapshot() async {
    try {
      ...
      logger.i(
        'Current day HRI snapshot saved: HRI ${_currentHRI.toStringAsFixed(1)}, Status: $hriStatus',
      );
    } catch (e) {
      logger.i('Error saving current day HRI snapshot: $e');
    }
  }
  ...
  Future<void> removeWorkout(String workoutId) async {
    final workoutToRemove = _todayWorkouts
