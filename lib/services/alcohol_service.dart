...
class AlcoholService extends ChangeNotifier {
  ...
  double get totalStandardDrinks {
    return _todayIntakes.fold(
      0.0,
      (sum, intake) => sum + intake.standardDrinks,
    );
  }
  ...
  double get totalWaterCorrection {
    return _todayIntakes.fold(
      0.0,
      (sum, intake) => sum + intake.getWaterCorrection(),
    );
  }
  ...
  double get totalSodiumCorrection {
    return _todayIntakes.fold(
      0.0,
      (sum, intake) => sum + intake.getSodiumCorrection(),
    );
  }
  ...
  Future<void> _saveToFirestore(AlcoholIntake intake) async {
    try {
      ...
      logger.i(
        'Alcohol intake saved to Firestore: ${intake.type} ${intake.standardDrinks}SD',
      );
    } catch (e) {
      logger.i('Error saving alcohol intake to Firestore: $e');
  ...
  Future<void> addIntake(AlcoholIntake intake) async {
    ...
    if (_isSameDay(intake.timestamp, DateTime.now())) {
      ...
      _achievementTracker.trackAlcoholIntake(
        ...
        completedRecovery:
            false, // Could be enhanced to track recovery completion
      );
  ...
  Future<void> removeIntake(String intakeId) async {
    ...
    final List<dynamic> intakesList = json.decode(intakesJson);
    List<Map<String, dynamic>> allIntakes = intakesList
  ...
  Future<void> _saveCheckinToFirestore(AlcoholCheckin checkin) async {
    try {
      ...
      logger.i(
        'Alcohol check-in ready for Firestore: ${checkin.date} (method not implemented yet)',
      );
    } catch (e) {
      logger.i('Error saving alcohol check-in to Firestore: $e');
  ...
  Future<Map<String, dynamic>> getWeeklyStats() async {
    ...
    final intakesJson = prefs.getString(_intakesKey);
    if (intakesJson == null) {
      return {'totalSD': 0.0, 'daysWithAlcohol': 0, 'soberDays': 7};
    }
    ...
    for (var intake in weekIntakes) {
      final dayKey =
          '${intake.timestamp.year}-${intake.timestamp.month}-${intake.timestamp.day}';
