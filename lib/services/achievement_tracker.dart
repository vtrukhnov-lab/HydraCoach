...
class AchievementTracker extends ChangeNotifier {
  ...
  final UnitsService _unitsService = UnitsService.instance; // ✅ ДОБАВЛЕНО: Интеграция с UnitsService
  ...
  Future<void> _saveStreakData() async {
    ...
    if (_lastStreakUpdateDate != null) {
      await prefs.setString('last_streak_update_date', _lastStreakUpdateDate!.toIso8601String());
    }
  ...
  bool hasNewInCategory(AchievementCategory category) {
    return _unlockedQueue.any((achievement) => achievement.category == category);
  }
  
  /// Получить количество новых достижений в категории
  int getNewCountInCategory(AchievementCategory category) {
    return _unlockedQueue.where((achievement) => achievement.category == category).length;
  }
  ...
  Future<void> markCategoryAsViewed(AchievementCategory category) async {
    ...
    _unlockedQueue.removeWhere((achievement) => achievement.category == category);
  ...
  Future<void> trackWaterIntake({
    ...
    if (now.hour < 9) {
      ...
      final currentProgressUserUnits = _convertVolumeToUserUnits(currentProgress.round());
    ...
    final literLegendProgressUserUnits = _convertVolumeToUserUnits(currentProgress.round());
  ...
  Future<void> trackElectrolyteIntake({
    ...
    // Salt Starter
    if (_electrolyteIntakesToday == 0 && (sodiumMg > 0 || potassiumMg > 0 || magnesiumMg > 0)) {
    ...
    if (!_reachedElectrolyteGoalsToday && 
        reachedSodiumGoal && reachedPotassiumGoal && reachedMagnesiumGoal) {
  ...
  Future<void> checkDailySugarAchievements() async {
    ...
    final sugarLimitUserUnits = _convertWeightToUserUnits(25.0); // 25г в единицах пользователя
  ...
  Future<void> checkMetaAchievements() async {
    ...
    for (final achievement in achievements) {
      if (achievement.isUnlocked && achievement.category != AchievementCategory.special) {
